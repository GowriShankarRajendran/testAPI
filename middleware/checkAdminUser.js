...
exports.checkAdminUser = ((req, res, next, role) => {
    ...
    if (checkTokenAvailable && getBearerIndex >= 0) {
        ...
        if (getToken) {
            server_1.database.query(`select userRole from userRegistration where userGUID = "${getToken.userGUID}"`, (err, result) => {
                if (err) {
                    res.status(500).json({ message: "Error", code: 500, value: err });
                }
                else {
                    if (result && result.length > 0) {
                        let getUserRole = result[0].userRole.split("#");
                        let checkUserAccess = false;
                        for (let i = 0; i < role.length; i++) {
                            checkUserAccess = getUserRole.includes(role[i]);
                            if (checkUserAccess) {
                                break;
                            }
                            else {
